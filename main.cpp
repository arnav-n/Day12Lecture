...
int main(int argc, char *argv[]) {
	...
	vector<shared_ptr<shape>> theShapes;
	cout << "the first shape: " << theShapes.at(1)->getDepth() << endl;

	//a triangle
	theShapes.push_back(make_shared<Polygon>(vec2(100, 30), 5, color(252)) );
	static_pointer_cast<Polygon>(theShapes.at(0))->addVert(vec2(120, 10));
	static_pointer_cast<Polygon>(theShapes.at(0))->addVert(vec2(145, 30));


	shared_ptr<Polygon> t2 = make_shared<Polygon>(vec2(140, 130), 5, color(252)) ;
	t2->addVert(vec2(160, 110));
	t2->addVert(vec2(185, 120));

	shared_ptr<Polygon> t3 = make_shared<Polygon>(vec2(140, 230), 5, color(252)) ;
